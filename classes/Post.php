...
class Post {
                ...
                if ($loggedInUserId == $profileUserId) {
                        ...
                        if (count(Notify::createNotify($postbody)) != 0) {
                                foreach (Notify::createNotify($postbody) as $key => $n) {
                                                ...
                                                if ($r != 0) {
                                                        DB::query('INSERT INTO notifications VALUES (LAST_INSERT_ID(), :type, :receiver, :sender, :extra)', array(':type'=>$n["type"], ':receiver'=>$r, ':sender'=>$s, ':extra'=>$n["extra"]));
                                                }
                                        }
                                }

                        DB::query('INSERT INTO posts VALUES (LAST_INSERT_ID(), :postbody, NOW(), :userid, 0, \'\', :topics)', array(':postbody'=>$postbody, ':userid'=>$profileUserId, ':topics'=>$topics));
        ...
        public static function createImgPost($postbody, $loggedInUserId, $profileUserId) {
                ...
                if ($loggedInUserId == $profileUserId) {
                        ...
                        DB::query('INSERT INTO posts VALUES (LAST_INSERT_ID(), :postbody, NOW(), :userid, 0, \'\', \'\')', array(':postbody'=>$postbody, ':userid'=>$profileUserId));
